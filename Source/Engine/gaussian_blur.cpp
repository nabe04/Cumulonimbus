	...
	:FullscreenQuad{device}
{
	...
	ps_blur_vertical   = std::make_unique<cumulonimbus::shader_system::PixelShader>(gaussian_blur_vertical_ps_name);
	ps_blur_horizontal = std::make_unique<cumulonimbus::shader_system::PixelShader>(gaussian_blur_horizontal_ps_name);
}
...
void GaussianBlur::GenerateGaussianBlur(ID3D11DeviceContext* immediate_context, ID3D11ShaderResourceView** const blur_texture)
{
	...
	ps_blur_horizontal->BindPS();
	FullscreenQuad::Blit(immediate_context);
	ps_blur_horizontal->UnbindPS();
	...
	ps_blur_vertical->BindPS();
	FullscreenQuad::Blit(immediate_context);
	ps_blur_vertical->UnbindPS();
