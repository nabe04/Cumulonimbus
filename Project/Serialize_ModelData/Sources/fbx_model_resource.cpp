...
//}
//
//FbxModelResource::FbxModelResource(ID3D11Device* device, const char* fbx_filename, const char* ignore_root_motion_node_name)
//{
...
//}
//
//void FbxModelResource::AddAnimation(const char* fbx_filename)
//{
//	FbxManager* fbx_manager = FbxManager::Create();
//
//	// FBXに対する入出力を定義する
...
//void FbxModelResource::BuildNodes(FbxNode* fbx_node, int parent_node_index)
//{
...
//
//// FBXノードからノードデータを構築する
//void FbxModelResource::BuildNode(FbxNode* fbx_node, int parent_node_index)
//{
//	// ノードのローカル変換行列を取得
...
//void FbxModelResource::BuildMeshes(ID3D11Device* device, FbxNode* fbx_node)
//{
//	FbxNodeAttribute*		fbx_node_attribute		= fbx_node->GetNodeAttribute();
//	FbxNodeAttribute::EType fbx_node_attribute_type	= FbxNodeAttribute::EType::eUnknown;
//
...
//void FbxModelResource::BuildMesh(ID3D11Device* device, FbxNode* fbx_node, FbxMesh* fbx_mesh)
//{
...
//
//		// 現在のマテリアルが「vecor<Material> materials」の何番目にあるかを検索
//		int index		= FindMaterialIndex(fbx_node->GetScene(), fbx_surface_material);
//		subset.material = &materials.at(index); // std::vector<Material> materialsの「index」番目の要素にアクセス
//	}
//
//	// サブセットの頂点インデックス範囲設定
...
//
//		buffer_desc.ByteWidth = static_cast<UINT>(sizeof(shader::Vertex) * vertices.size());
...
//		D3D11_SUBRESOURCE_DATA subresource_data = {};
//
//		buffer_desc.ByteWidth = static_cast<UINT>(sizeof(u_int) * indices.size());
//		//buffer_desc.Usage = D3D11_USAGE_DEFAULT;
//		buffer_desc.Usage = D3D11_USAGE_IMMUTABLE;
...
//void FbxModelResource::BuildMaterials(ID3D11Device* device, const char* dirname, FbxScene* fbx_scene)
//{
...
//void FbxModelResource::BuildMaterial(ID3D11Device* device, const char* dirname, FbxSurfaceMaterial* fbx_surface_material)
//{
//	bool ret = false;
//
//	Material material;
...
//
//// アニメーションデータを構築
//void FbxModelResource::BuildAnimations(FbxScene* fbx_scene)
//{
//	// すべてのアニメーション名を取得
...
//
//// ノードインデックスを取得する
//int FbxModelResource::FindNodeIndex(const char* name)
//{
//	for (size_t i = 0; i < nodes.size(); ++i)
//	{
...
//
//// マテリアルインデックスを取得する
//int FbxModelResource::FindMaterialIndex(FbxScene* fbx_scene, const FbxSurfaceMaterial* fbx_surface_material)
//{
//	int fbx_material_count = fbx_scene->GetMaterialCount();
//
